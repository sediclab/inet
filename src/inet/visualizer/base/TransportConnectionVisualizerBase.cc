...
TransportConnectionVisualizerBase::Connection::Connection(int sourceModuleId, int destinationModuleId, int count) :
...
void TransportConnectionVisualizerBase::addConnection(const Connection *connection)
{
    connections.push_back(connection);
}

void TransportConnectionVisualizerBase::removeConnection(const Connection *connection)
{
...
void TransportConnectionVisualizerBase::receiveSignal(cComponent *source, simsignal_t signal, cObject *object DETAILS_ARG)
{
    if (signal == inet::tcp::TCP::tcpConnectionAddedSignal) {
        ...
        {
            addConnection(createConnection(source, destination, tcpConnection));
        }
    }
}
